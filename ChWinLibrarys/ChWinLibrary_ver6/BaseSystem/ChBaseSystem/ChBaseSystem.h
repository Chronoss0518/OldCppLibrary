...
namespace ChSystem
{
	...
	public:
		...
		//Windの横幅サイズ取得//
		const inline unsigned int GetWindWidth()const { return WindSize[0]; }

		//Windの立幅サイズ取得//
		const inline unsigned int GetWindHeight()const { return WindSize[1]; }

		const inline ChMath::VectorBase<unsigned int, 2> GetWindSize()const { return WindSize; }
	...
	protected:
		...
		ChMath::VectorBase<unsigned int, 2>WindSize;
	...
	public:
		...
		ChVec2 GetWindSize()
		{
			if (BaseSystems == nullptr)return ChVec2();

			ChVec2 Out;

			Out.Val.Set(BaseSystems->GetWindSize());
