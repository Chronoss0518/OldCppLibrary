...
namespace ChStd
{
	...
	struct FPOINT
	{
		...
		FPOINT& operator =(const FPOINT _Pos)
		{
			x = _Pos.x;
			y = _Pos.y;
			return *this;
		...
		FPOINT operator +(const FPOINT _Pos)const
		{
			FPOINT Tmp = *this;
			Tmp += _Pos;
			return Tmp;
		...
		FPOINT operator =(const float _Num)const
		{
			FPOINT Tmp = *this;
	...
	struct COLOR1f
	{
		COLOR1f& operator= (const float _Num)
		{
			a = r = g = b = _Num;
			return *this;
		}

		COLOR1f& operator=(const COLOR255&);

		COLOR1f& operator= (const COLOR1f& _Col)
		{
			a = _Col.a;
			r = _Col.r;
			g = _Col.g;
	...
	struct COLOR255
	{
		COLOR255& operator= (const unsigned char _Num)
		{
			a = r = g = b = _Num;
			return *this;
		}

		COLOR255& operator=(const COLOR1f&);

		COLOR255& operator= (const COLOR255& _Col)
		{
			a = _Col.a;
			r = _Col.r;
			g = _Col.g;
	...
	//プロセスの実行時間//
	inline unsigned long GetClock() { return static_cast<unsigned long>(std::clock()); }

	//開始時に使用し、ランダムな値をセットする//
	inline void SetRand(const unsigned long _UseNo) { srand(_UseNo); }

	//SetRand使用後rand関数から値を取り出す//
	inline unsigned short GetRand(const unsigned short _Max)
	{
	...
	//角度が0から360以内に収まっているか//
	//収まっていない場合-5だった場合は355へと置き換わる//
	inline void CheckOverDegree(float* _Degree)
	{
	...
	inline Bool FPSProcess(const unsigned long _FPS)
	{
	...
	inline auto EnumCast(Enum _Enum)->typename std::enable_if
