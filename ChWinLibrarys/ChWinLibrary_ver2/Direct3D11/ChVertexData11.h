...
namespace ChD3D11
{
	...
	struct SpriteData
	{
		...
		{
			SpData.Ver[0].Pos = ChVec3_11(_Pos.x, _Pos.y, 0.0f);
			SpData.Ver[1].Pos = ChVec3_11(_Pos.x + _Size.w, _Pos.y, 0.0f);
			SpData.Ver[2].Pos = ChVec3_11(_Pos.x + _Size.w, _Pos.y + _Size.h, 0.0f);
			SpData.Ver[3].Pos = ChVec3_11(_Pos.x, _Pos.y + _Size.h, 0.0f);
		}

		SpriteData(const  ChStd::FRECT& _Rect)
		{
			SpData.Ver[0].Pos = ChVec3_11(_Rect.x, _Rect.y, 0.0f);
			SpData.Ver[1].Pos = ChVec3_11(_Rect.x + _Rect.w, _Rect.y, 0.0f);
			SpData.Ver[2].Pos = ChVec3_11(_Rect.x + _Rect.w, _Rect.y + _Rect.h, 0.0f);
			SpData.Ver[3].Pos = ChVec3_11(_Rect.x, _Rect.y + _Rect.h, 0.0f);
		}
