...
namespace ChMath
{
	...
	inline float ToDegree(const float _Radian) { return (_Radian * 180.0f / Pi); }

	//DegreeäpÇ©ÇÁRadianäpÇ÷//
	inline float ToRadian(const float _Degree) { return (_Degree * Pi / 180.0f); }

	//ê≥ÇÃïÑçÜÇ©Ç«Ç§Ç©ÇämîFÇ∑ÇÈ//
	inline ChStd::Bool IsPSign(const int _Val) { return _Val >= 0.0f ? true : false; }
	inline ChStd::Bool IsPSign(const char _Val) { return _Val >= 0.0f ? true : false; }
	inline ChStd::Bool IsPSign(const short _Val) { return _Val >= 0.0f ? true : false; }
	inline ChStd::Bool IsPSign(const long _Val) { return _Val >= 0.0f ? true : false; }
	inline ChStd::Bool IsPSign(const float _Val) { return _Val >= 0.0f ? true : false; }
	inline ChStd::Bool IsPSign(const double _Val) { return _Val >= 0.0f ? true : false; }
